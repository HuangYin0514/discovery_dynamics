...
def visualize_one_state(pos_truth, pos_baseline, pos_hnn):
    ...
    for i in range(t_max - 3):
        plt.plot(pos_truth[i:i + 2, 2], pos_truth[i:i + 2, 3], 'k-', label='_nolegend_', linewidth=LINE_WIDTH,
                 alpha=0.2 + 0.8 * (i + 1) / t_max)  # 轨迹
        if i % 200 == 0:  # 中间状态
            plt.plot([0, pos_truth[i, 0]], [0, pos_truth[i, 1]], color='brown', linewidth=LINE_WIDTH,
                     label='_nolegend_', alpha=0.2 + 0.8 * (i + 1) / t_max)
            plt.plot([pos_truth[i, 0], pos_truth[i, 2]], [pos_truth[i, 1], pos_truth[i, 3]], 'o-', color='brown',
                     linewidth=LINE_WIDTH, label='_nolegend_',
                     alpha=0.2 + 0.8 * (i + 1) / t_max)
            plt.scatter(pos_truth[i, 0], pos_truth[i, 1], s=50, linewidths=LINE_WIDTH, facecolors='gray',
            ...
            plt.scatter(pos_truth[i, 2], pos_truth[i, 3], s=50, linewidths=LINE_WIDTH, facecolors='gray',
                        edgecolors='brown', label='_nolegend_',
                        alpha=min(0.5 + 0.8 * (i + 1) / t_max, 1), zorder=3)
    plt.plot(pos_truth[t_max - 2:t_max, 2], pos_truth[t_max - 2:t_max, 3], 'k-', label='Ground truth',
             linewidth=LINE_WIDTH, alpha=1)  # 轨迹
    plt.plot([0, pos_truth[t_max, 0]], [0, pos_truth[t_max, 1]], color='brown', linewidth=LINE_WIDTH,
             label='_nolegend_')
    plt.plot([pos_truth[t_max, 0], pos_truth[t_max, 2]], [pos_truth[t_max, 1], pos_truth[t_max, 3]], 'o-',
             color='brown', linewidth=LINE_WIDTH, label='Pendulum')  # 摆
    plt.scatter(pos_truth[t_max, 0], pos_truth[t_max, 1], s=50, linewidths=LINE_WIDTH, facecolors='gray',
                edgecolors='brown', label='_nolegend_', alpha=1, zorder=3)
    plt.scatter(pos_truth[t_max, 2], pos_truth[t_max, 3], s=50, linewidths=LINE_WIDTH, facecolors='gray',
                edgecolors='brown', label='_nolegend_', alpha=1, zorder=3)  # 球
    ...
    for i in range(t_max - 3):
        plt.plot(pos_truth[i:i + 2, 2], pos_truth[i:i + 2, 3], 'k-', label='_nolegend_', linewidth=LINE_WIDTH,
                 alpha=0.2 + 0.8 * (i + 1) / t_max)
        plt.plot(pos_baseline[i:i + 2, 2], pos_baseline[i:i + 2, 3], 'r-.', label='_nolegend_', linewidth=LINE_WIDTH,
                 alpha=0.2 + 0.8 * (i + 1) / t_max)  # 轨迹
        if i % 200 == 0:
            plt.plot([0, pos_baseline[i, 0]], [0, pos_baseline[i, 1]], color='brown', linewidth=LINE_WIDTH,
                     label='_nolegend_', alpha=0.2 + 0.8 * (i + 1) / t_max)
            plt.plot([pos_baseline[i, 0], pos_baseline[i, 2]], [pos_baseline[i, 1], pos_baseline[i, 3]], 'o-',
                     color='brown', linewidth=2, label='_nolegend_',
                     alpha=0.2 + 0.8 * (i + 1) / t_max)
            ...
            plt.scatter(pos_baseline[i, 2], pos_baseline[i, 3], s=50, linewidths=2, facecolors='gray',
                        ...
                        zorder=3)
    plt.plot(pos_truth[t_max - 2:t_max, 2], pos_truth[t_max - 2:t_max, 3], 'k-', label='Ground truth',
             linewidth=LINE_WIDTH, alpha=1)  # Ground truth 轨迹
    plt.plot(pos_baseline[t_max - 2:t_max, 2], pos_baseline[t_max - 2:t_max, 3], 'r-.', label='baseline',
             linewidth=LINE_WIDTH, alpha=1)  # hnn 轨迹
    plt.plot([0, pos_baseline[t_max, 0]], [0, pos_baseline[t_max, 1]], color='brown', linewidth=2, label='_nolegend_')
    plt.plot([pos_baseline[t_max, 0], pos_baseline[t_max, 2]], [pos_baseline[t_max, 1], pos_baseline[t_max, 3]], 'o-',
             color='brown', linewidth=2, label='Pendulum')  # 摆
    plt.scatter(pos_baseline[t_max, 0], pos_baseline[t_max, 1], s=50, linewidths=2, facecolors='gray',
                edgecolors='brown', label='_nolegend_', alpha=1, zorder=3)
    plt.scatter(pos_baseline[t_max, 2], pos_baseline[t_max, 3], s=50, linewidths=2, facecolors='gray',
                edgecolors='brown', label='_nolegend_', alpha=1, zorder=3)  # 摆末端的点
    plt.legend(fontsize=LEGENDSIZE)
    ...
    for i in range(t_max - 3):
        plt.plot(pos_truth[i:i + 2, 2], pos_truth[i:i + 2, 3], 'k-', label='_nolegend_', linewidth=LINE_WIDTH,
                 alpha=0.2 + 0.8 * (i + 1) / t_max)
        plt.plot(pos_hnn[i:i + 2, 2], pos_hnn[i:i + 2, 3], 'g-.', label='_nolegend_', linewidth=LINE_WIDTH,
                 alpha=0.2 + 0.8 * (i + 1) / t_max)  # 轨迹
        if i % 200 == 0:
            plt.plot([0, pos_hnn[i, 0]], [0, pos_hnn[i, 1]], color='brown', linewidth=LINE_WIDTH, label='_nolegend_',
                     alpha=0.2 + 0.8 * (i + 1) / t_max)
            plt.plot([pos_hnn[i, 0], pos_hnn[i, 2]], [pos_hnn[i, 1], pos_hnn[i, 3]], 'o-', color='brown', linewidth=2,
                     label='_nolegend_', alpha=0.2 + 0.8 * (i + 1) / t_max)
    ...
    plt.plot(pos_truth[t_max - 2:t_max, 2], pos_truth[t_max - 2:t_max, 3], 'k-', label='Ground truth',
             linewidth=LINE_WIDTH, alpha=1)  # Ground truth 轨迹
    plt.plot(pos_hnn[t_max - 2:t_max, 2], pos_hnn[t_max - 2:t_max, 3], 'g-.', label='HNN', linewidth=LINE_WIDTH,
             alpha=1)  # hnn 轨迹
    plt.plot([0, pos_hnn[t_max, 0]], [0, pos_hnn[t_max, 1]], color='brown', linewidth=2, label='_nolegend_')
    plt.plot([pos_hnn[t_max, 0], pos_hnn[t_max, 2]], [pos_hnn[t_max, 1], pos_hnn[t_max, 3]], 'o-', color='brown',
             linewidth=2, label='Pendulum')  # 摆
    plt.scatter(pos_hnn[t_max, 0], pos_hnn[t_max, 1], s=50, linewidths=2, facecolors='gray', edgecolors='brown',
                label='_nolegend_', alpha=1, zorder=3)
    plt.scatter(pos_hnn[t_max, 2], pos_hnn[t_max, 3], s=50, linewidths=2, facecolors='gray', edgecolors='brown',
                label='_nolegend_', alpha=1, zorder=3)  # 摆末端的点
    plt.legend(fontsize=LEGENDSIZE)
