...
class MyDataset(ln.Data):
    ...
    def lagrangian_get_ddq(self, coords):
        ...
        A = np.linalg.inv(A)
        B = np.asarray([
            [- self.l[0] * (self.l[1] * self.m[1] * np.sin(th1 - th2) * dth1 * dth2 + self.g * self.m[0] * np.sin(
                th1) + +self.g * self.m[1] * np.sin(th1)),
             self.l[1] * self.m[1] * (self.l[0] * np.sin(th1 - th2) * dth1 * dth2 - self.g * np.sin(th2))]])
        C = np.asarray([
            [-1 * self.l[0] * self.l[1] * self.m[1] * np.sin(th1 - th2) * dth2,
             self.l[0] * self.l[1] * self.m[1] * np.sin(th1 - th2) * dth2],
            [-1 * self.l[0] * self.l[1] * self.m[1] * np.sin(th1 - th2) * dth1,
             self.l[0] * self.l[1] * self.m[1] * np.sin(th1 - th2) * dth1]
        ])
    ...
    def get_dataset(self, seed=0, samples=100, ode_stepsize=0.1, test_split=0.9, system='hnn', **kwargs):
        ...
        if system == 'hnn':
            ...
            self.X_train = data['x']
            self.y_train = data['dx']
            self.X_test = data['test_x']
            self.y_test = data['test_dx']
