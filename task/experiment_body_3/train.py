...
from data_body_3 import MyDataset
...
def get_args():
    ...
    # MODEL SETTINGS
    parser.add_argument('--model', default='hnn', type=str, help='Select model to train')
    ...
    parser.add_argument('--end_epoch', default=11, type=int, help='end of training epoch')
...
def train_Baseline(args):
    ...
    if os.path.exists(filename):
        ...
        dataset = MyDataset(obj=args.obj, m=[1 for i in range(args.obj)], l=[1 for i in range(args.obj)])
...
def train_HNN(args):
    ...
    if args.plot:
        fig = plt.figure()
        plt.semilogy((stats['loss_train']), 'b', label='train loss')
        plt.semilogy((stats['loss_test']), 'r', label='test loss')
        plt.legend()
        # plt.show()
        path = '{}/fig-{}-{}-{}-hidden_dim-{}-start_epoch-{}-end_epoch-{}-noise-{}-learn_rate-{}.{}'.format(
            args.save_dir, args.obj, args.name,
...
def train_LNN(args):
    ...
    optim = torch.optim.Adam(
        ...
        betas=(0.9, 0.999))
    lambda1 = lambda \
            epoch: 1  # if epoch < 800 else 0.4 if epoch < 3000 else 0.2 if epoch < 6000 else 0.1 if epoch < 9000 else 0.04 if epoch < 10000 else 0.02 if epoch < 11000 else 0.01
    scheduler_enc = torch.optim.lr_scheduler.LambdaLR(optim, lr_lambda=lambda1)
    print('number of parameters in model: ', count_parameters(model))
