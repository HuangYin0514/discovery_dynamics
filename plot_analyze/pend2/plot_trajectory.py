...
def plot_pend_trajectory(true_q,HnnModScale_q, baseline_q,HNN_q):
    ...
    for i in range(time - 3):
        ...
        plt.plot(HnnModScale_pos[i:i + 2, 2], HnnModScale_pos[i:i + 2, 3], 'g-.', label='_nolegend_', linewidth=2,
                 alpha=0.2 + 0.8 * (i + 1) / time)
        if i % 200 == 0:
            plt.plot([0, HnnModScale_pos[i, 0]], [0, HnnModScale_pos[i, 1]], color='brown', linewidth=2, label='_nolegend_',
                     alpha=0.2 + 0.8 * (i + 1) / time)
            plt.plot([HnnModScale_pos[i, 0], HnnModScale_pos[i, 2]], [HnnModScale_pos[i, 1], HnnModScale_pos[i, 3]], 'o-',
    ...
    plt.plot(HnnModScale_pos[time - 2:time, 2], HnnModScale_pos[time - 2:time, 3], 'g-.', label='SMHNet (Ours)',
             linewidth=2, alpha=1)
    plt.plot([0, HnnModScale_pos[time, 0]], [0, HnnModScale_pos[time, 1]], color='brown', linewidth=2, label='_nolegend_')
    plt.plot([HnnModScale_pos[time, 0], HnnModScale_pos[time, 2]], [HnnModScale_pos[time, 1], HnnModScale_pos[time, 3]], 'o-',
             color='brown', linewidth=2, label='Pendulum')
    plt.scatter(HnnModScale_pos[time, 0], HnnModScale_pos[time, 1], s=50, linewidths=2, facecolors='gray', edgecolors='brown',
                label='_nolegend_', alpha=1, zorder=3)
    plt.scatter(HnnModScale_pos[time, 2], HnnModScale_pos[time, 3], s=50, linewidths=2, facecolors='gray', edgecolors='brown',
                label='_nolegend_', alpha=1, zorder=3)
    ...
    for i in range(time - 3):
        ...
        if i % 200 == 0:
            plt.plot([0, baseline_pos[i, 0]], [0, baseline_pos[i, 1]], color='brown', linewidth=2, label='_nolegend_',
                     alpha=0.2 + 0.8 * (i + 1) / time)
            plt.plot([baseline_pos[i, 0], baseline_pos[i, 2]], [baseline_pos[i, 1], baseline_pos[i, 3]], '-', color='brown',
                     linewidth=2, label='_nolegend_', alpha=0.2 + 0.8 * (i + 1) / time)
            plt.scatter(baseline_pos[i, 0], baseline_pos[i, 1], s=50, linewidths=2, facecolors='gray', edgecolors='brown',
                        label='_nolegend_', alpha=min(0.5 + 0.8 * (i + 1) / time, 1), zorder=3)
            plt.scatter(baseline_pos[i, 2], baseline_pos[i, 3], s=50, linewidths=2, facecolors='gray', edgecolors='brown',
                        label='_nolegend_', alpha=min(0.5 + 0.8 * (i + 1) / time, 1), zorder=3)
    plt.plot(truth_pos[time - 2:time, 2], truth_pos[time - 2:time, 3], 'k-', label='Ground truth', linewidth=2, alpha=1)
    plt.plot(baseline_pos[time - 2:time, 2], baseline_pos[time - 2:time, 3], 'r:', label='Baseline', linewidth=2, alpha=1)
    plt.plot([0, baseline_pos[time, 0]], [0, baseline_pos[time, 1]], color='brown', linewidth=2, label='_nolegend_')
    plt.plot([baseline_pos[time, 0], baseline_pos[time, 2]], [baseline_pos[time, 1], baseline_pos[time, 3]], 'o-', color='brown',
             linewidth=2, label='Pendulum')
