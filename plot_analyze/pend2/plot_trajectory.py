...
def plot_pend_trajectory_Cartesian(truth_pos, SCLNN_pos):
    fig = plt.figure(figsize=(16, 4), dpi=DPI)
    ...
    for i in range(time - 3):
        plt.plot(truth_pos[i:i + 2, 2], truth_pos[i:i + 2, 3], 'k-', label='_nolegend_', linewidth=2,
                 alpha=0.2 + 0.8 * (i + 1) / time)
        plt.plot(SCLNN_pos[i:i + 2, 2], SCLNN_pos[i:i + 2, 3], 'g-.', label='_nolegend_', linewidth=2,
                 alpha=0.2 + 0.8 * (i + 1) / time)
        if i % 200 == 0:
            plt.plot([0, SCLNN_pos[i, 0]], [0, SCLNN_pos[i, 1]], color='brown', linewidth=2,
                     label='_nolegend_',
                     alpha=0.2 + 0.8 * (i + 1) / time)
            plt.plot([SCLNN_pos[i, 0], SCLNN_pos[i, 2]], [SCLNN_pos[i, 1], SCLNN_pos[i, 3]],
                     'o-',
                     color='brown', linewidth=2, label='_nolegend_', alpha=0.2 + 0.8 * (i + 1) / time)
            plt.scatter(SCLNN_pos[i, 0], SCLNN_pos[i, 1], s=50, linewidths=2, facecolors='gray',
                        edgecolors='brown', label='_nolegend_', alpha=min(0.5 + 0.8 * (i + 1) / time, 1), zorder=3)
            plt.scatter(SCLNN_pos[i, 2], SCLNN_pos[i, 3], s=50, linewidths=2, facecolors='gray',
                        edgecolors='brown', label='_nolegend_', alpha=min(0.5 + 0.8 * (i + 1) / time, 1), zorder=3)
    plt.plot(truth_pos[time - 2:time, 2], truth_pos[time - 2:time, 3], 'k-', label='Ground truth', linewidth=2, alpha=1)
    plt.plot(SCLNN_pos[time - 2:time, 2], SCLNN_pos[time - 2:time, 3], 'g-.', label='SMHNet (Ours)',
             linewidth=2, alpha=1)
    plt.plot([0, SCLNN_pos[time, 0]], [0, SCLNN_pos[time, 1]], color='brown', linewidth=2,
             label='_nolegend_')
    plt.plot([SCLNN_pos[time, 0], SCLNN_pos[time, 2]], [SCLNN_pos[time, 1], SCLNN_pos[time, 3]],
             'o-',
             color='brown', linewidth=2, label='Pendulum')
    plt.scatter(SCLNN_pos[time, 0], SCLNN_pos[time, 1], s=50, linewidths=2, facecolors='gray',
                edgecolors='brown',
                label='_nolegend_', alpha=1, zorder=3)
    plt.scatter(SCLNN_pos[time, 2], SCLNN_pos[time, 3], s=50, linewidths=2, facecolors='gray',
                edgecolors='brown',
                label='_nolegend_', alpha=1, zorder=3)
